...
export default function ProfilePage() {
  ...
  useEffect(() => {
    if (hasHydrated && !isAuthenticated) router.push('/login?redirect=/profile')
      ...
      <div className="space-y-6 animate-fade-in font-['Inter']">
        ...
        <div className="grid grid-cols-3 gap-3 px-2">
            <div className="bg-white p-3 rounded-[16px] border border-accent-100 shadow-sm text-center">
                <div className="text-xs text-gray-400 mb-1 font-bold bg-vanilla-50 py-0.5 rounded uppercase tracking-wider">LEVEL</div>
                <div className="font-bold text-primary-600 text-[18px] font-['Poppins']">{user.gamification_level}</div>
            </div>
            <div className="bg-white p-3 rounded-[16px] border border-accent-100 shadow-sm text-center">
                <div className="text-xs text-gray-400 mb-1 font-bold bg-vanilla-50 py-0.5 rounded uppercase tracking-wider">ĐIỂM</div>
                <div className="font-bold text-accent-600 text-[18px] font-['Poppins']">{user.total_points.toLocaleString()}</div>
            </div>
            <div className="bg-white p-3 rounded-[16px] border border-accent-100 shadow-sm text-center">
                <div className="text-xs text-gray-400 mb-1 font-bold bg-vanilla-50 py-0.5 rounded uppercase tracking-wider">THÂM NIÊN</div>
        ...
        <Card className="p-0 overflow-hidden border border-accent-100 shadow-sm animate-slide-up">
          {[
            ...
            { icon: Calendar, label: 'Ngày vào làm', value: formatDate(user.hire_date) },
             ...
             <div key={idx} className="flex items-center gap-4 p-4 border-b border-gray-100 last:border-none hover:bg-vanilla-50 transition-colors">
        ...
        <Card className="p-0 overflow-hidden border border-accent-100 shadow-sm animate-slide-up">
          {[
             ...
             <div key={idx} onClick={() => router.push(item.href)} className="flex items-center justify-between p-4 border-b border-gray-100 last:border-none hover:bg-vanilla-50 cursor-pointer group transition-all">
