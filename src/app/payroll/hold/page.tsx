      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2 animate-slide-up">
          {mockSalaryHolds.map(h => (
            <div key={h.id} className="card flex items-start gap-3" style={{ borderLeft: `4px solid ${h.status === 'holding' ? '#F6C85F' : '#1E9E57'}` }}>
              <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
                style={{ background: h.status === 'holding' ? '#F6C85F20' : '#1E9E5720' }}>
                {h.status === 'holding' ? <Pause size={18} className="text-warning-500" /> : <CheckCircle size={18} className="text-emerald-500" />}
              </div>
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>{h.employee_name}</span>
                  <span className="text-sm font-bold" style={{ color: h.status === 'holding' ? '#F6C85F' : '#1E9E57' }}>{fmt(h.hold_amount)}</span>
                </div>
                <div className="text-xs" style={{ color: 'var(--text-muted)' }}>Giữ {h.hold_percent}%</div>
                <div className="flex gap-2 mt-1">
                  <span className="text-[9px] px-1.5 py-0.5 rounded-full" style={{ background: h.status === 'holding' ? '#F6C85F20' : '#1E9E5720', color: h.status === 'holding' ? '#F6C85F' : '#1E9E57' }}>
