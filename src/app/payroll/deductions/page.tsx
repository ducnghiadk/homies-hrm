...
export default function DeductionSlipPage() {
  ...
  const [deductions, setDeductions] = useState<TicketItem[]>([])
  ...
  const [ticketType, setTicketType] = useState('Phiếu phạt')
  const [customTicketType, setCustomTicketType] = useState('Phạt Lỗi Vận Hành')
  const [targetEmpId, setTargetEmpId] = useState(mockEmployees[0]?.id || '')
  const [ticketScope, setTicketScope] = useState('date')
  ...
  const loadRealData = async () => {
    setIsLoading(true)
    try {
      const realTx = await payrollAdapter.getTicketTransactions(false)
      setDeductions(realTx as unknown as TicketItem[])
    } catch (err) {
      console.error('Error loading real deduction slips:', err)
    } finally {
      setIsLoading(false)
    }
  }
  ...
  // Save Deduction Ticket Handler directly to Supabase DB
  const handleSaveDeduction = async () => {
    ...
    const newType = ticketType.includes('tự định nghĩa') ? `Phiếu trừ tự định nghĩa: ${customTicketType}` : ticketType
    ...
    alert(`Đã lưu phiếu trừ tiền cho nhân viên ${targetEmp.full_name} trực tiếp vào CSDL Supabase!`)
    setShowModal(false)
    setTicketAmount(0)
    setTicketNote('')
    setTicketImage(null)
  ...
  // Filter Logic
  const filteredList = deductions.filter(item => {
    if (filterType !== 'all' && !item.ticket_type.includes(filterType)) return false
    return true
  })

  return (
    <AppShell title="Phiếu trừ tiền">
      <input
        type="file"
        ref={fileInputRef}
        accept="image/*"
      ...
      <div className="space-y-4 text-xs">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 p-2 bg-slate-900 text-white rounded-2xl shadow-sm border border-slate-800">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <select
              ...
              className="bg-slate-800 border border-slate-700 text-white text-xs rounded-xl px-3 py-1.5 font-medium focus:outline-none cursor-pointer"
            >
              <option value="all">Tất cả loại phiếu trừ</option>
        ...
        {/* DATA TABLE (REAL DB DATA FROM SUPABASE) */}
          ...
          <table className="w-full text-left text-xs border-collapse whitespace-nowrap">
            <thead>
              <tr className="bg-slate-100 text-slate-700 font-bold border-b border-slate-200 text-[11px] uppercase">
                ...
                <th className="p-3 border-r border-slate-200 min-w-[140px]">Hình thức trừ</th>
                <th className="p-3 border-r border-slate-200 text-right min-w-[120px]">Giá trị trừ đi</th>
            ...
            <tbody className="divide-y divide-slate-200 bg-white">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={11} className="p-8 text-center text-slate-400 font-medium">
                    Chưa có phiếu trừ tiền nào trong CSDL Supabase. Hãy bấm nút <strong>&quot;+ Thêm mới&quot;</strong> ở góc phải để tạo phiếu phạt thật!
                  </td>
                </tr>
              ) : (
                filteredList.map((row, index) => (
                  <tr key={row.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="p-3 border-r border-slate-100 text-right font-mono font-black text-rose-600 text-sm">
                      -{fmt(row.amount)}
                    </td>
      ...
      {/* FULL INTERACTIVE MODAL: THÊM PHIẾU TRỪ TIỀN */}
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/65 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full overflow-hidden border border-gray-100 transition-all">
            <div className="px-6 py-4.5 border-b border-gray-100 flex items-center justify-between sticky top-0 z-10 bg-white">
              <h3 className="text-lg font-bold text-gray-900">Thêm phiếu trừ tiền</h3>
              <button
            ...
            <div className="p-6 space-y-4.5 max-h-[82vh] overflow-y-auto text-xs">
              ...
              {ticketType === 'Phiếu trừ tiền tự định nghĩa' && (
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <label className="block text-gray-800 font-bold text-xs">
                      Chọn loại phiếu trừ tiền tự định nghĩa <span className="text-rose-500">*</span>
                    </label>
                    <button
                      type="button"
                  ...
                  <select
                    ...
                    onChange={e => {
                      ...
                      if (val === 'Phạt Lỗi Vận Hành') setTicketAmount(5000)
              ...
              <div>
                <label className="block text-gray-800 font-bold mb-1.5 text-xs">
                  Nhân viên bị trừ <span className="text-rose-500">*</span>
                </label>
                <select
                  value={targetEmpId}
                  ...
                  className="w-full p-3 rounded-xl border border-gray-200 bg-white text-gray-800 font-semibold focus:border-sky-500 focus:outline-none cursor-pointer text-xs shadow-2xs"
                >
                  <option value="">Chọn nhân viên bị trừ</option>
                  {mockEmployees.map(e => (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-gray-800 font-bold mb-1.5 text-xs">
                    Trừ tiền cho <span className="text-rose-500">*</span>
                  </label>
                  <select
                    value={ticketScope}
                ...
                <div>
                  <label className="block text-gray-800 font-bold mb-1.5 text-xs">
                    {ticketScope === 'period' ? 'Chu kỳ áp dụng *' : 'Ngày bị trừ *'}
                  </label>
                  {ticketScope === 'period' ? (
                    <select
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-gray-800 font-bold mb-1.5 text-xs">
                    Hình thức trừ <span className="text-rose-500">*</span>
                  </label>
                  <select
                    ...
                    className="w-full p-3 rounded-xl border border-gray-200 bg-white text-gray-800 font-semibold focus:border-sky-500 focus:outline-none cursor-pointer text-xs shadow-2xs"
                  >
                    <option value="amount">Trừ tiền</option>
                    <option value="percent">Trừ phần trăm lương</option>
                  </select>
                </div>

                <div>
                  <label className="block text-gray-800 font-bold mb-1.5 text-xs">
                    Giá trị trừ đi <span className="text-rose-500">*</span>
                  </label>
              ...
              <div className="p-3.5 rounded-xl bg-sky-50/90 border border-sky-100 text-sky-900 leading-relaxed text-xs font-medium space-y-1">
                ...
                <div className="text-[11px] text-sky-700 font-semibold">
                  Nếu chi nhánh là Công ty ➔ Phiếu trừ tiền sẽ trừ vào Lương theo công ty của nhân viên
                </div>
              </div>
