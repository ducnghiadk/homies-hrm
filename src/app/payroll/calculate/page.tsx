...
import {
  ...
  ChevronDown, ChevronUp, Users, DollarSign, Clock,
} from 'lucide-react'
...
export default function PayrollCalculationPage() {
  ...
  const summary = useMemo(() => {
    ...
    return { totalNet, totalGross, totalInsurance, totalTax, count: results.length }
  }, [results])
      ...
      <div className="space-y-4">
            ...
            {steps.map((step, i) => {
              ...
              return (
                <div key={i} className="flex items-start gap-2 p-1.5 rounded-lg transition-all"
                  style={{
                    background: isDone ? '#10b98108' : isCurrent ? 'var(--primary-50)' : 'var(--gray-50)',
                    ...(isCurrent ? { boxShadow: '0 0 0 1px var(--primary-200)' } : {}),
                  }}>
                  <div className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5"
                    style={{
                      background: isDone ? '#10b98120' : isCurrent ? 'var(--primary-100)' : 'var(--gray-100)',
                  ...
                  <div>
                    <div className="text-xs font-medium" style={{ color: isDone ? '#10b981' : 'var(--text-primary)' }}>{step.label}</div>
        ...
        {summary && (
          <div className="space-y-3 animate-fade-in">
            <div className="card" style={{ background: '#10b98110', border: '1px solid #10b98130' }}>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs font-medium text-red-500">{fmt(summary.totalInsurance + summary.totalTax)}</div>
                </div>
              </div>
            </div>
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-[10px]" style={{ color: 'var(--text-muted)' }}>
                            ...
                            {r.late_count > 0 && <span className="text-amber-500"> • {r.late_count} trễ</span>}
                            {r.leave_days_paid > 0 && <span className="text-blue-500"> • {r.leave_days_paid} phép</span>}
                            {r.absent_days > 0 && <span className="text-red-500"> • {r.absent_days} vắng</span>}
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                      ...
                      {isOpen && (
                        <div className="p-2.5 pt-0 space-y-2" style={{ background: 'var(--gray-50)' }}>
                          ...
                          <div className="grid grid-cols-4 gap-1 text-center">
                            ...
                            <div className="rounded p-1" style={{ background: 'var(--bg-main)' }}>
                              <div className="text-xs font-bold text-blue-500">{r.leave_days_paid}</div>
                              <div className="text-[9px]" style={{ color: 'var(--text-muted)' }}>Phép</div>
                            </div>
                            <div className="rounded p-1" style={{ background: 'var(--bg-main)' }}>
                              <div className="text-xs font-bold text-amber-500">{r.late_count}</div>
                              <div className="text-[9px]" style={{ color: 'var(--text-muted)' }}>Trễ</div>
                            </div>
                            <div className="rounded p-1" style={{ background: 'var(--bg-main)' }}>
                              <div className="text-xs font-bold text-red-500">{r.absent_days}</div>
                          ...
                          <div className="space-y-0.5 text-xs">
                            ...
                            {r.overtime_hours > 0 && <Row label="OT" value={fmt(r.overtime_amount)} sub={`${r.overtime_hours}h × 1.5x`} color="#f59e0b" />}
                            {r.total_allowances > 0 && <Row label="Phụ cấp" value={fmt(r.total_allowances)} />}
                            {r.total_bonuses > 0 && <Row label="Thưởng" value={fmt(r.total_bonuses)} color="#10b981" />}
                          ...
                          <div className="space-y-0.5 text-xs">
                            {r.late_deduction > 0 && <Row label={`Trễ (${r.late_count}×50k)`} value={`-${fmt(r.late_deduction)}`} color="#ef4444" />}
                            {r.absent_deduction > 0 && <Row label="Vắng không phép" value={`-${fmt(r.absent_deduction)}`} color="#ef4444" />}
                            {r.advance_deduction > 0 && <Row label="Ứng lương" value={`-${fmt(r.advance_deduction)}`} color="#ef4444" />}
                            {r.other_deductions.map((d, i) => (
                              <Row key={i} label={d.name} value={`-${fmt(d.amount)}`} color="#ef4444" />
                            ))}
                            <Row label="BHXH (8%)" value={`-${fmt(r.bhxh)}`} color="#ef4444" />
                            <Row label="BHYT (1.5%)" value={`-${fmt(r.bhyt)}`} color="#ef4444" />
                            <Row label="BHTN (1%)" value={`-${fmt(r.bhtn)}`} color="#ef4444" />
                            {r.tax > 0 && <Row label="Thuế TNCN" value={`-${fmt(r.tax)}`} color="#ef4444" />}
                          </div>

                          {/* Net */}
                          <div className="rounded-lg p-2 mt-2" style={{ background: '#10b98110', border: '1px solid #10b98130' }}>
        ...
        {!results && (
          <div className="card animate-slide-up" style={{ animationDelay: '0.1s' }}>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-bold text-amber-500">Lưu ý trước khi tính</span>
            </div>
