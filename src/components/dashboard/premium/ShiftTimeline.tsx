...
export function ShiftTimeline({ shift, onCheckIn }: Props) {
  ...
  const cfg: Record<string, { gradient: string; icon: string; text: string; color: string }> = {
    ...
    current: { gradient: 'from-emerald-50 to-teal-50', icon: '🟢', text: 'Đang trong ca làm việc', color: 'text-emerald-700' },
    upcoming: { gradient: 'from-blue-50 to-indigo-50', icon: '🕐', text: 'Sắp đến ca', color: 'text-blue-700' },
  };
    ...
    <div className={`bg-gradient-to-br ${c.gradient} rounded-2xl p-5 border border-white/50`}>
      ...
      <div className="relative mb-4">
        ...
        <div className="h-3 bg-white/60 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-purple-500 to-indigo-500 rounded-full transition-all duration-1000 relative" style={{ width: `${progress}%` }}>
            {shift.status === 'current' && <div className="absolute right-0 top-1/2 -translate-y-1/2 w-4 h-4 bg-white rounded-full shadow-lg border-2 border-purple-500" />}
          </div>
        </div>
      </div>

      {shift.status === 'current' && (
        <p className="text-center text-sm text-gray-600 mb-4">Còn <span className="font-semibold text-purple-600">{remH}h {remMin}m</span> nữa</p>
      )}

      <div className="flex gap-3">
        {shift.status === 'upcoming' && onCheckIn && (
          <button onClick={onCheckIn} className="flex-1 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-medium rounded-xl hover:shadow-lg hover:shadow-purple-500/30 transition-all flex items-center justify-center gap-2">
            <span>📍</span><span>Check-in</span>
          </button>
        )}
        <a href="/schedule" className="flex-1 py-3 bg-white text-gray-700 font-medium rounded-xl border border-gray-200 hover:border-purple-300 transition-all flex items-center justify-center gap-2">
