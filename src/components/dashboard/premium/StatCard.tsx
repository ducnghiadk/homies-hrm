...
export function StatCard({ value, label, icon, trend, href, highlight = false, size = 'md' }: Props) {
  ...
  const tColors: Record<string, string> = { up: 'text-emerald-600 bg-emerald-50', down: 'text-red-600 bg-red-50', neutral: 'text-gray-600 bg-gray-50' };
  const tIcons: Record<string, string> = { up: '↑', down: '↓', neutral: '→' };

  const content = (
    <div className={`rounded-2xl p-4 transition-all duration-300 ${highlight ? 'bg-gradient-to-br from-purple-500 to-indigo-600 text-white shadow-lg shadow-purple-500/30' : 'bg-white border border-gray-100 hover:border-purple-200 hover:shadow-md'}`}>
      <div className="flex items-start justify-between mb-2">
        {icon && <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg ${highlight ? 'bg-white/20' : 'bg-purple-50'}`}>{icon}</div>}
        {trend && (
