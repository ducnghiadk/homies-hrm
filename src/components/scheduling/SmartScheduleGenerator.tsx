...
import {
  ...
  AlertTriangle, RotateCw, Settings, Heart, BellRing
} from 'lucide-react'
...
// Preference-aware generator & history (NEW)
import { generatePreferenceAwareSchedule } from '@/lib/scheduling/preference-aware-generator'
import { getMissingPreferenceEmployees } from '@/lib/scheduling/preference-aware-scheduler'
import { saveScheduleGeneration } from '@/lib/scheduling/generation-history'
import { notifyMissingPreferences } from '@/lib/notifications/preference-notifications'
import { useDeviceType } from '@/lib/design-system/breakpoints'
import { toast } from 'sonner'

// Result View
import ScheduleResultView from './ScheduleResultView'
import ScheduleView from '@/components/staffing/ScheduleView'

// Drag-Drop Editor
import DragDropScheduleEditor from './DragDropScheduleEditor'

interface Props {
  prefilledConfig?: unknown
...
export default function SmartScheduleGenerator({ onPublish }: Props) {
  // State
  ...
  const [trafficPattern] = useState<HourlyTrafficPattern[]>(MOCK_TRAFFIC_PATTERN)
  ...
  const handleGenerate = async () => {
    ...
    if (staffList.filter(s => !s.unavailableDates?.includes('ALL')).length === 0) {
        toast.error('Cần ít nhất 1 nhân viên khả dụng')
    ...
    try {
        const schedule = generatePreferenceAwareSchedule(job)
        const savedSchedule = saveScheduleGeneration(format(weekStart, 'yyyy-MM-dd'), schedule)
        setResult(savedSchedule)
    } catch (e) {
        console.error(e)
        toast.error('Có lỗi xảy ra khi tạo lịch', {
          description: 'Vui lòng thử lại sau khi kiểm tra dữ liệu nhân viên và ràng buộc.',
        })
    } finally {
        setIsGenerating(false)
    }
  ...
  const renderStep2_Staff = () => {
    const missingPrefList = getMissingPreferenceEmployees(staffList, weekStart, [])
    const handleSendReminders = () => {
      if (missingPrefList.length === 0) return

      const weekStartStr = format(weekStart, 'yyyy-MM-dd')
      const weekEndStr = format(weekEnd, 'yyyy-MM-dd')
      const { sent, skipped } = notifyMissingPreferences({
        userIds: missingPrefList.map(staff => staff.employeeId),
        weekStart: weekStartStr,
        weekEnd: weekEndStr,
      })

      if (sent > 0 && skipped > 0) {
        setReminderFeedback(`Đã gửi ${sent} nhắc nhở, bỏ qua ${skipped} người vì tuần này đã được nhắc trước đó.`)
      } else if (sent > 0) {
        setReminderFeedback(`Đã gửi nhắc nhở đăng ký ca cho ${sent} nhân viên.`)
      } else {
        setReminderFeedback('Tất cả nhân viên thiếu preference tuần này đã được nhắc rồi.')
      }
    }
    ...
    return (
      <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4">
        ...
        {missingPrefList.length > 0 && (
          <div className="bg-warning-50 border border-warning-200/50 p-3 rounded-xl flex gap-2.5 text-xs text-warning-800 animate-in slide-in-from-top-2">
            <Heart size={16} className="text-warning-600 shrink-0 mt-0.5" />
            <div className="flex-1">
              <span className="font-semibold">{missingPrefList.length} nhân viên</span> chưa đăng ký nguyện vọng tuần này ({missingPrefList.map(s => s.name).join(', ')}).
              <div className="mt-1 text-[10px] text-warning-600 opacity-90">Hệ thống sẽ tự động xếp ca tối ưu dựa trên lịch sử nếu không có nguyện vọng mới.</div>
              {reminderFeedback && (
                <div className="mt-2 text-[10px] font-semibold text-warning-700">
                  {reminderFeedback}
                </div>
              )}
            </div>
            <button
              ...
              className="shrink-0 self-start px-3 py-2 rounded-xl bg-white text-warning-700 border border-warning-200 hover:bg-warning-100 transition-colors font-bold flex items-center gap-1.5"
            >
              <BellRing size={12} /> Nhắc ngay
            </button>
          </div>
        )}
            ...
            <div key={staff.employeeId} className="flex items-center justify-between p-3 bg-white border border-gray-100 rounded-xl hover:border-primary/50 transition-colors">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="text-xs text-muted-foreground flex items-center gap-2">
                            <span className={`px-1.5 py-0.5 rounded ${staff.type === 'fulltime' ? 'bg-primary-50 text-primary-700' : 'bg-warning-50 text-warning-700'}`}>
                ...
                <div className="text-right">
                    ...
                    {staff.unavailableDates && staff.unavailableDates.length > 0 && !staff.unavailableDates.includes('ALL') && (
                        <div className="text-xs text-error-500 mt-1">Nghỉ {staff.unavailableDates.length} ngày</div>
                    )}
                </div>
            </div>
        ))}
  ...
  const renderStep3_Constraints = () => (
    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4">
       <div className="bg-primary-50 p-3 rounded-lg flex gap-3 text-sm text-primary-800">
  ...
  const renderStep4_Traffic = () => (
    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4">
                ...
                {Array.from({ length: 16 }, (_, i) => i + 7).map(h => (
                    <div key={h} className="flex border-b last:border-0 text-xs">
                        ...
                        {Array.from({ length: 7 }, (_, d) => {
                            ...
                            if (demand >= 4) bgClass = 'bg-error-100 text-error-700 font-bold'
                            else if (demand === 3) bgClass = 'bg-warning-50 text-warning-600'
        ...
        <div className="bg-warning-50 p-3 rounded-lg text-xs text-warning-800 flex gap-2">
  ...
  if (result && isEditing) {
    return (
      <DragDropScheduleEditor
        key={`${result.id}-${result.generatedAt}`}
        schedule={result}
        staffList={staffList}
  ...
  if (result) {
    if (deviceType !== 'desktop') {
      return (
        <ScheduleView
          schedule={result}
          onPublish={result.status === 'draft' && onPublish ? () => onPublish(result) : undefined}
        />
      )
    }
