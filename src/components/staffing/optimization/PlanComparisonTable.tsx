...
export default function PlanComparisonTable({ plans, selectedId }: PlanComparisonTableProps) {
  return (
                 ...
                 return (
                    <td key={plan.id} className="p-4 text-center">
                        <div className="flex justify-center gap-1">
                            {[1,2,3,4,5].map(i => (
                                <div key={i} className={`w-2 h-2 rounded-full ${i <= (score * 5) ? 'bg-primary-500' : 'bg-gray-200'}`} />
                            ))}
                        </div>
                    </td>
                 );
                ...
                <ul className="text-xs text-gray-600 space-y-1">
                    {plan.pros.map((pro, i) => (
                        <li key={i} className="flex gap-1">
                            <Check size={12} className="text-success-500 mt-0.5 shrink-0"/> {pro}
                        </li>
                    ))}
                </ul>
              </td>
             ...
             {plans.map(plan => (
              <td key={plan.id} className="p-4 align-top">
                <ul className="text-xs text-gray-500 space-y-1">
                    {plan.cons.map((con, i) => (
                        <li key={i} className="flex gap-1">
                            <AlertTriangle size={12} className="text-warning-400 mt-0.5 shrink-0"/> {con}
                        </li>
                    ))}
                </ul>
              </td>
