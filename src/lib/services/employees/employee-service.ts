...
const DEFAULT_APP_BASE_URL = 'http://localhost:3535'
...
export function getDepartmentName(positionId?: string, role?: AuthUser['role']) {
  if (['ceo', 'hr_admin'].includes(role || 'employee')) return 'Ban Giám đốc & Điều hành'
  const positionName = mockPositions.find(position => position.id === positionId)?.name || ''
  if (positionName.includes('Quản lý') || positionName.includes('Trưởng ca') || role === 'store_manager' || role === 'shift_leader') {
    return 'Quản lý cửa hàng'
  }
  if (positionName.includes('Pha chế') || positionId === 'pos-001') return 'Pha chế & Barista'
  if (positionName.includes('Thu ngân') || positionId === 'pos-002') return 'Thu ngân & Sales'
  if (positionName.includes('Phục vụ') || positionId === 'pos-003') return 'Phục vụ & Sảnh'
  return 'Vận hành cửa hàng'
}

export function getDefaultSecondaryPositions(positionId?: string, role?: AuthUser['role']): string[] {
  if (positionId === 'pos-001') return ['pos-002'] // Pha chế -> kiêm Thu ngân
  if (positionId === 'pos-002') return ['pos-001'] // Thu ngân -> kiêm Pha chế
  return ['pos-001', 'pos-002'] // Tất cả vị trí khác -> kiêm cả Pha chế & Thu ngân
}
...
function sanitizeEmployeeRecord(value: unknown, index: number): AuthUser | null {
  ...
  const employee: AuthUser = {
    ...
    secondary_position_ids: Array.isArray(value.secondary_position_ids) ? value.secondary_position_ids : seed?.secondary_position_ids,
    secondary_store_ids: Array.isArray(value.secondary_store_ids) ? value.secondary_store_ids : seed?.secondary_store_ids,
...
function getExtraMetadataFallback(user: AuthUser): { secondary_position_ids?: string[], secondary_store_ids?: string[], department_name?: string } {
  if (typeof window === 'undefined') return {}
  try {
    const raw = localStorage.getItem('HOMIES_EMPLOYEE_EXTRA_METADATA_V1')
    if (!raw) return {}
    const map = JSON.parse(raw) as Record<string, { secondary_position_ids?: string[], secondary_store_ids?: string[], department_name?: string }>
    return (user.id && map[user.id]) ||
           (user.employee_code && map[user.employee_code]) ||
           (user.email && map[user.email.toLowerCase()]) || {}
  } catch {
    return {}
  }
}
...
function normalizeEmployeeProfile(user: AuthUser): AuthUser {
  const extra = getExtraMetadataFallback(user)
  ...
  const department_name = user.department_name ?? extra.department_name ?? getDepartmentName(user.position_id, user.role)
  ...
  const secondary_position_ids = Array.isArray(user.secondary_position_ids) && user.secondary_position_ids.length > 0
    ? user.secondary_position_ids
    : Array.isArray(extra.secondary_position_ids) && extra.secondary_position_ids.length > 0
      ? extra.secondary_position_ids
      : getDefaultSecondaryPositions(user.position_id, user.role)
  const secondary_store_ids = Array.isArray(user.secondary_store_ids) && user.secondary_store_ids.length > 0
    ? user.secondary_store_ids
    : Array.isArray(extra.secondary_store_ids) && extra.secondary_store_ids.length > 0
      ? extra.secondary_store_ids
      : []
...
// Danh sách nhân sự chính thức của hệ thống Homies
const INITIAL_EMPLOYEES: AuthUser[] = [
  {
    id: 'emp-001',
    full_name: 'Nguyễn Đức Nghĩa',
  ...
  {
    id: 'e04b5c70-97e2-4061-8bef-0fae5563a066',
    full_name: 'Huỳnh Lê Kiều Linh',
    email: 'huynhlekieulinh6@gmail.com',
    role: 'employee',
    store_id: 'store-002',
    position_id: 'pos-001',
    phone: '0779554540',
    employee_code: 'BH-0913',
    status: 'active',
    account_status: 'dang_hoat_dong',
    total_points: 1500,
  ...
  {
    id: 'f747f06f-9e95-406e-a5b5-0f6571dbfaa4',
    full_name: 'Phạm Nguyễn Đông Duy',
    email: 'phamnguyendongduy2020@gmail.com',
    role: 'employee',
    store_id: 'store-002',
    position_id: 'pos-001',
    phone: '0943749525',
    employee_code: 'NV0028',
    status: 'active',
    account_status: 'dang_hoat_dong',
    total_points: 1800,
    gamification_level: 'silver',
  ...
  {
    id: '2f3acef5-118f-490b-8530-d86aa164b90a',
    full_name: 'Lê Minh Lộc',
    email: 'lloclm0203@gmail.com',
    role: 'employee',
    store_id: 'store-002',
    position_id: 'pos-001',
    phone: '0945576422',
    employee_code: 'NV0020',
    status: 'active',
    account_status: 'dang_hoat_dong',
    total_points: 1650,
    gamification_level: 'silver',
  ...
  {
    id: 'd8b8ecd5-1e74-415b-a739-2fa6bbd81284',
    full_name: 'Nguyễn Thanh Thiện',
    email: 'nguyenthanhthien2804@gmail.com',
    role: 'employee',
    store_id: 'store-002',
    position_id: 'pos-002',
    phone: '0775943568',
    employee_code: 'NV0017',
    status: 'active',
    account_status: 'dang_hoat_dong',
    total_points: 1700,
    gamification_level: 'silver',
    hire_date: '2025-10-21',
    kpi_level: 'L2',
  },
  {
    id: '10a4897f-9936-41a7-804f-8a4556eaaf0c',
    full_name: 'Nguyễn Thị Kiều Ý',
    email: 'kieuy4052007@gmail.com',
    role: 'store_manager',
    store_id: 'store-002',
    position_id: 'pos-005',
    phone: '0378410208',
  ...
  {
    id: '9392ab72-eacc-4211-a0b3-a183c0db75d5',
    full_name: 'Trần Công Huy',
    email: 'jinn.wooo.02@gmail.com',
    role: 'store_manager',
    store_id: 'store-001',
    position_id: 'pos-005',
    phone: '0332151527',
    employee_code: 'NV0027',
    status: 'active',
    account_status: 'dang_hoat_dong',
    total_points: 2200,
    gamification_level: 'gold',
...
export class EmployeeService {
  ...
  private static mergeSeedEmployees(data: AuthUser[]): AuthUser[] {
    const obsoleteDummyIds = new Set(['emp-002', 'emp-003', 'emp-004', 'emp-005', 'emp-006', 'emp-007', 'emp-012', 'emp-013', 'emp-014', 'emp-015', 'emp-016', 'emp-006-vy', 'emp-022-hang', 'emp-023-phuong'])
    const merged = data
      .map((user, index) => sanitizeEmployeeRecord(user, index))
      .filter((user): user is AuthUser => user !== null && !obsoleteDummyIds.has(user.id))

    INITIAL_EMPLOYEES.forEach(seedUser => {
      const existingIndex = merged.findIndex(
        user => (user.id && seedUser.id && user.id === seedUser.id) ||
                (user.email && seedUser.email && user.email.toLowerCase() === seedUser.email.toLowerCase()) ||
                (user.employee_code && seedUser.employee_code && user.employee_code === seedUser.employee_code) ||
                (user.full_name && seedUser.full_name && user.full_name.trim().toLowerCase() === seedUser.full_name.trim().toLowerCase())
      )
    ...
    // Khử trùng lặp triệt để theo employee_code, email, id và full_name
    ...
    return deduped.map(user => normalizeEmployeeProfile(user))
  }
  ...
  static syncEmployeesFromAdapter(employees: AuthUser[]) {
    if (typeof window === 'undefined' || !employees || employees.length === 0) return
    const db = this.getDb()
    const merged = [...db]
    employees.forEach(emp => {
      const idx = merged.findIndex(e => e.id === emp.id || (emp.email && e.email.toLowerCase() === emp.email.toLowerCase()))
      if (idx === -1) {
        merged.push(emp)
      } else {
  ...
  static resolveSessionUser(user?: AuthUser | Pick<AuthUser, 'id' | 'email'> | null): AuthUser | undefined {
    ...
    if ('role' in user && user.role) {
      ...
      try {
        const db = this.getDb()
        const exists = db.some(e => e.id === authUser.id || (authUser.email && e.email.toLowerCase() === authUser.email.toLowerCase()))
        if (!exists) {
          this.saveDb([...db, authUser])
        }
      } catch (e) {
        console.warn('[EmployeeService] Auto-sync session user failed:', e)
      }
      return authUser
    }
  ...
  static getEmployeeById(id: string, currentUser?: AuthUser): AuthUser | undefined {
    ...
    const raw = db.find(e =>
      e.id === id ||
      e.employee_code === id ||
      (e.email && id && e.email.toLowerCase() === id.toLowerCase())
    )

    if (!raw) return undefined
    const employee = normalizeEmployeeProfile(raw)
  ...
  static createEmployee(data: Partial<AuthUser>, currentUser?: AuthUser, source: 'manual' | 'import' | 'invitation' = 'manual'): AuthUser {
    ...
    const newId = data.id || `emp-${Date.now()}-${nextNum}`
    const newCode = data.employee_code || `BH-${nextNum.toString().padStart(3, '0')}`
    ...
    const newEmp: AuthUser = {
      ...
      position_id: data.position_id || 'pos-007',
      phone: data.phone || '',
  ...
  static updateEmployee(id: string, updates: Partial<AuthUser>, currentUser?: AuthUser, reason?: string): AuthUser | null {
    ...
    if (index === -1) {
      index = db.findIndex(e =>
        e.employee_code === id ||
        (updates.employee_code && e.employee_code === updates.employee_code) ||
        (e.email && updates.email && e.email.toLowerCase() === updates.email.toLowerCase()) ||
        (e.email && id && e.email.toLowerCase() === id.toLowerCase())
      )
    }
  ...
  static resetEmployeePassword(id: string, newPassword?: string, currentUser?: AuthUser): { success: boolean; password?: string; message?: string } {
    const db = this.getDb()
    const index = db.findIndex(e => e.id === id)
    if (index === -1) return { success: false, message: 'Nhân sự không tồn tại' }

    const generatedPassword = newPassword && newPassword.trim() 
      ? newPassword.trim() 
      : `Homies@${Math.floor(1000 + Math.random() * 9000)}`

    db[index] = normalizeEmployeeProfile({
      ...db[index],
      password: generatedPassword,
    } as AuthUser & { password?: string })
    this.saveDb(db)
    ...
    this.appendEmployeeActivityLog({
      ...
      detail: `Đã cấp mật khẩu mới: ${generatedPassword}`,
      actor_id: currentUser?.id,
      actor_name: currentUser?.full_name,
    })

    return { success: true, password: generatedPassword, message: 'Đã đặt lại mật khẩu thành công' }
  }
