...
export function getEmployeeOnboardingChecklistPlans(): EmployeeOnboardingChecklistPlan[] {
  return [..._onboardingEmployeePlans].sort((left, right) => left.created_at.localeCompare(right.created_at));
}
...
export function getEmployeeOnboardingChecklistBundleByEmployeeId(employeeId: string) {
  const employee = mockEmployees.find((entry) => entry.id === employeeId);
  if (!employee) {
    return null;
  }

  return getEmployeeOnboardingChecklistBundleForEmployee(employee);
}

export function updateEmployeeOnboardingChecklistProgressItem(
  planId: string,
  checklistItemId: string,
  updater: (current: EmployeeOnboardingChecklistProgressItem) => EmployeeOnboardingChecklistProgressItem,
): EmployeeOnboardingChecklistProgressItem | null {
  const current = _onboardingEmployeeProgressItems.find((item) =>
    item.onboarding_plan_id === planId && item.checklist_item_id === checklistItemId
  );
  ...
  _onboardingEmployeeProgressItems = _onboardingEmployeeProgressItems.map((item) =>
    item.id === current.id ? next : item
  );
  save(KEYS.onboardingEmployeeProgressItems, _onboardingEmployeeProgressItems);

  const planIndex = _onboardingEmployeePlans.findIndex((item) => item.id === planId);
  if (planIndex !== -1) {
    _onboardingEmployeePlans[planIndex] = {
      ..._onboardingEmployeePlans[planIndex],
      updated_at: today(),
