-- Homies Milk Tea - secure RLS for operational and payroll data.
-- Apply after schema_v3_master_fixed.sql.
...
-- Keep legacy callers safe if an old policy still exists in a partially migrated database.
...
CREATE POLICY hrm_nhan_vien_update ON public.nhan_vien
  ...
  USING (
    ...
    OR (
      public.app_hrm_can_manage_store(cua_hang_id)
      AND to_chuc_id = public.app_hrm_current_org_id()
    )
  )
  WITH CHECK (
    ...
    OR (
      ...
      AND (
        public.app_hrm_is_payroll_admin()
        OR vai_tro::TEXT IN ('nhan_vien', 'truong_ca', 'quan_ly_cua_hang')
      )
    )
  );
