...
- File chinh: `docs/superpowers/plans/*`, `docs/superpowers/plans/2026-05-25-noi-quy-onboarding-flow-plan.md`, `docs/superpowers/plans/2026-05-28-onboarding-3-mat-plan.md`, `docs/superpowers/plans/2026-05-28-onboarding-output-standard-plan.md`, `docs/superpowers/plans/2026-05-28-onboarding-self-review-plan.md`, `docs/superpowers/plans/2026-05-28-onboarding-stage-gate-plan.md`, `docs/superpowers/plans/2026-05-31-pass-e-onboarding-role-settings-plan.md`
- Dung khi: da duyet spec va can bat dau code theo tung task co thu tu ro rang, nhat la rollout `chuan dau ra onboarding theo vi tri` vao data mock, service, 3 man onboarding, pass `self-review theo chang` / `gate tong ket chang`, hoac Pass E role settings onboarding

## Config build local
### Root config de Next nhin dung workspace
- Mo ta: bo config toi thieu de `next build` va alias `@/*` chay dung trong scratch workspace nay, tranh bi Next/Turbopack nhin sang root khac.
- File chinh: `package.json`, `tsconfig.json`, `next.config.ts`, `postcss.config.mjs`
- File lien quan root chung: `C:/Users/Admin/.gemini/antigravity/postcss.config.cjs`
- Dung khi: build bi lech root, alias `@/*` khong resolve, PostCSS/Tailwind plugin sai, hoac can smoke nhanh route `/onboarding` va `/career-path/onboarding`
